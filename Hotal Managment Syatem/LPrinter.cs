...
namespace Hotal_Managment_Syatem
{
    ...
    class LPrinter
    {
        ...
        public int qty;
        public string date, wname, time, counterNo, datefrm, date_To, tableno;
        public float billid, rate, GrandTot ,discAmt, TotalAmt, Cgstamt, Sgstamt;
        public string item_nm, date1, date2, connection_str, amt, billDate, billtime;
        public int kotID = 0;
        ...
        public LPrinter(string con_string)
        {
            HandlePrinter = IntPtr.Zero;
            ps = new PrinterSettings();
        ...
        public LPrinter(string table_no, string date, float billid, string name, string time)
        {
 ...
 inner join category cat on cat.cat_name=menu.category
  WHERE (sales_item.order_id = '" + this.billid + "') and (cat.BillPrinter=2 )");
              ...
              if (getFoodItemCount != 0)
              {
                  ...
                  pdoc.PrintPage += new PrintPageEventHandler(this.temp_pdoc_PrintPage);
                  pdoc.Print();
              ...
              {
                  pdoc.PrintPage += new PrintPageEventHandler(this.temp_pdoc_PrintPage);  //test the new code 12-1-2014 by sagar
                  pdoc.Print();
              }
 ...
 WHERE sales_item.order_id = '" + this.billid + "' group by menu.printName,sales_item.rate,menu.category,cat.BillPrinter  ORDER BY min(sales_item.sales_id)";
        ...
        public void today_counterEpson(object sender, PrintPageEventArgs e)
        {
            ...
            if (grupwseflg)
            {
                if (db.ChkDb_Value("select value from tbl_option where grp='PrintSize' and value='3 Inch'"))
                {
                    ...
                    graphics.DrawString(" \t              Total : " + (this.GrandTot + this.discAmt - (this.Cgstamt * 2) - creditrec) + "\r\n", new Font(fontName, 9, System.Drawing.FontStyle.Bold), new SolidBrush(Color.Black), startX, startY + Offset);
                    if (creditrec!=0)
                    {
                    Offset = Offset + 20;
                    graphics.DrawString("         Credit Received : " + this.creditrec + "\r\n", new Font(fontName, 9, System.Drawing.FontStyle.Bold), new SolidBrush(Color.Black), startX, startY + Offset);
                    }
                    Offset = Offset + 20;
                    graphics.DrawString("  \t       Discount : " + this.discAmt + "\r\n", new Font(fontName, 9, System.Drawing.FontStyle.Bold), new SolidBrush(Color.Black), startX, startY + Offset);
                    Offset = Offset + 20;
        ...
        public void Expences(object sender, PrintPageEventArgs e)
        {
            ...
            graphics.DrawString("----------------------------------\r\n", new Font("Courier New", 10, System.Drawing.FontStyle.Bold), new SolidBrush(Color.Black), startX, startY + Offset);
            Offset = Offset + 20;
            graphics.DrawString(" Expences    Total      Remark  \r\n", new Font("Courier New", 10, System.Drawing.FontStyle.Bold), new SolidBrush(Color.Black), startX, startY + Offset);
            Offset = Offset + 20;
            ...
            while (rd.Read() == true)
            {
                ...
                Offset = Offset + 20;
                graphics.DrawString("" + mName + "   " + total + "  " + remark  + "\r\n", new Font("Courier New", 10, System.Drawing.FontStyle.Bold), new SolidBrush(Color.Black), startX, startY + Offset);
        ...
        public string menuQry = "", fromDate, toDate, categoryType , menutype;
     
        DataGridView drinkgrid;
     
        //yogesh 30/04/2019

        public void print_stockreport(DataGridView drinks, string menu)
        {
            menutype = menu;
            drinkgrid = drinks;
            cn.Open();
            ...
            pdoc.DefaultPageSettings.PaperSize.Width = 520;
               PrinterSettings printer = new PrinterSettings();
            pdoc.PrinterSettings.PrinterName = (PrinterName);
            pdoc.PrintPage += new PrintPageEventHandler(stockkreport);
        ...
        public void stockkreport(object sender, PrintPageEventArgs e)
        {
            get_ReceiptFormt(); //get the receipt format from the db
            Database db = new Database();
            string get_suggestion = "";
            ...
            int startY = 30;
            int Offset = 0;
               graphics.DrawString("\t   Stock  Report\r\n", new Font(fontName, 13, System.Drawing.FontStyle.Bold), new SolidBrush(Color.Black), startX, startY + Offset);
        
            Offset = Offset + 20;
            graphics.DrawString("      Category : " + menutype + "\t Time : " + System.DateTime.Now.ToString("hh:mm:ss") + "\r\n", new Font(fontName, 9, System.Drawing.FontStyle.Bold), new SolidBrush(Color.Black), startX, startY + Offset);

            Offset = Offset + 20;
            graphics.DrawString("   Date From : " + System.DateTime.Now.ToString("dd/MM/yyyy") + "    \r\n", new Font(fontName, 9, System.Drawing.FontStyle.Bold), new SolidBrush(Color.Black), startX, startY + Offset);
            ...
            Offset = Offset + 20;
            graphics.DrawString(" Item Name        Open_Qty      sale_Qty     balance  \r\n", new Font(fontName, 8, FontStyle.Bold), new SolidBrush(Color.Black), startX, startY + Offset);
            Offset = Offset + 10;
            graphics.DrawString("---------------------------------------------------------------------\r\n", new Font(fontName, 8, System.Drawing.FontStyle.Bold), new SolidBrush(Color.Black), startX, startY + Offset);


            string sr = i.ToString();
            int count = drinkgrid.RowCount - 1;
            for (int j = 0; j <= count; j++)
            {
                mname = drinkgrid.Rows[j].Cells[0].Value.ToString();
                qty = drinkgrid.Rows[j].Cells[1].Value.ToString();
                total = drinkgrid.Rows[j].Cells[2].Value.ToString();
                string balance = drinkgrid.Rows[j].Cells[3].Value.ToString();
        ...
        public void print_drinkreport(DataGridView drinks, string menu)
        {
            menutype = menu;
            drinkgrid = drinks;
            //fromDate = dateFrm;
            ...
            PrinterSettings printer = new PrinterSettings();
            pdoc.PrinterSettings.PrinterName = (PrinterName);
            pdoc.PrintPage += new PrintPageEventHandler(drinkreport);
        ...
        public void drinkreport(object sender, PrintPageEventArgs e)
        {
            get_ReceiptFormt(); //get the receipt format from the db
            Database db = new Database();
            string get_suggestion = "";
            ...
            //if (db.ChkDb_Value("select value from tbl_option where grp='PrintSize' and value='3 Inch'"))
            //{
            graphics.DrawString("\t  Drink Stock  Report\r\n", new Font(fontName, 13, System.Drawing.FontStyle.Bold), new SolidBrush(Color.Black), startX, startY + Offset);
            //Offset = Offset + 20;
        ...
        DataGridView dvgsales;
        int start, end;
        public void print_reportEpson(string menuqry, string dtfrm, string dtto,DataGridView dvggrid,int min,int max)
        {
            frmDt = dtfrm;
            ToDt = dtto;
            dvgsales = dvggrid;
        ...
        public void reportMenuDetail(object sender, PrintPageEventArgs e)
        {
           ...
           // SqlCommand cmd = new SqlCommand(menudtlqry, cn);
            ...
            {
                mname = dvgsales.Rows[j].Cells[0].Value.ToString();
                qty = dvgsales.Rows[j].Cells[1].Value.ToString();
                salestotal = dvgsales.Rows[j].Cells[2].Value.ToString();
            ...
            if (flgSales)
            {
                if (db.ChkDb_Value("select value from tbl_option where grp='PrintSize' and value='3 Inch'"))
                {
                    ...
                    Offset = Offset + 20;
                    graphics.DrawString("\t\t    Total=" + salesto + "\r\n", new Font(fontName, 9, System.Drawing.FontStyle.Bold), new SolidBrush(Color.Black), startX, startY + Offset);
        ...
        public void print_DailyReportSummery(object sender, PrintPageEventArgs e)
        {
            ...
            getValueDetails = db.Displaygrid(@" select 'Total Billing :' as Header,SUM(total_bill)  as amount from total_bill where  datetime between '" + Convert.ToDateTime(reportFromdate).ToString("MM-dd-yyyy") + "' AND '" + Convert.ToDateTime(reportToDate).ToString("MM-dd-yyyy") + "'  AND status<>'NC' "
                                + " union all select 'Total '+status+' :' as Header,SUM(total_bill) as amount from total_bill where  datetime between '" + Convert.ToDateTime(reportFromdate).ToString("MM-dd-yyyy") + "' AND '" + Convert.ToDateTime(reportToDate).ToString("MM-dd-yyyy") + "' AND status<>'NC'  and  order_id<>200   group by status");
            ...
            graphics.DrawString("Total Credit cash :                       " + this.cash + "  \r\n", new Font(fontName, 11, System.Drawing.FontStyle.Bold), new SolidBrush(Color.Black), startX, startY + Offset);

            Offset = Offset + 20;


            graphics.DrawString("Total Credit card :                        " + this.card + " \r\n ", new Font(fontName, 11, System.Drawing.FontStyle.Bold), new SolidBrush(Color.Black), startX, startY + Offset);
...
//            getValueDetails = db.Displaygrid(@"select 'Total '+cat.FoodSection +' Sales :' as Header,sum(total_amount) as amount from sales_item si inner join menu m on si.menu_id=m.menu_id
//inner join category cat on m.category=cat.cat_name 
//inner join total_bill tob on tob.order_id=si.order_id  and datetime between '" + Convert.ToDateTime(reportFromdate).ToString("MM-dd-yyyy") + "' AND '" + Convert.ToDateTime(reportToDate).ToString("MM-dd-yyyy") + "'  group by cat.FoodSection    ");
